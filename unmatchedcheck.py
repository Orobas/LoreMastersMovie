...
with open("UnmatchedData.tsv") as f:
...
with open("LeftoverWords.tsv","w") as write:
		write.write("word\tpostive count\tnegative count\ttotalscore\taccuracy\n")
		for key in totalwords:
			write.write(key + "\t" + str(poswords[key]) + "\t" + str(negwords[key]) + "\t" + str(totalwords[key]) + "\t" + str(max(int(poswords[key]),int(negwords[key]))/(int(poswords[key])+int(negwords[key]))) + "\n")
		write.close()
